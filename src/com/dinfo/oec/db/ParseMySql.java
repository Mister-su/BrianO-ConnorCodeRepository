...
public class ParseMySql implements ParseDb {
    private static final Logger log = Logger.getLogger(ParseMySql.class);
	...
	public Multimap<String, String> getConceptResByType(int type)
	{
		...
		String sql ="select  CONCAT(CAST(tree_id AS CHAR),'_',concept_id) cate ,resource   from concept_resource"+DbConfig.suffix+"  where type=? " +
				"  and concept_id is not null and concept_id <>'' and resource is not null and resource<>'' ";
		try {
	...
	{
		Multimap<String, String> res = ArrayListMultimap.create();
		String sql ="select  CONCAT(CAST(tree_id AS CHAR),'_',element_id) cate ,resource  res from element_resource"+DbConfig.suffix+" where type=?  " +
				"  and element_id is not null and element_id<>'' and resource is not null and resource <>'' ";
	...
	public BidiMap getOntologyMap()
	{
		...
		String sql ="select  CONCAT(CAST(tree_id AS CHAR),'_',ontology_id) cate ,ontology_name  res from ontology"+DbConfig.suffix+"  " +
				"  where ontology_id is not null and  ontology_id <>'' and ontology_name is not null and ontology_name <>''";
		try {
	...
	public Map<String, String> getOntologyPath() {
		...
		String sql ="select  CONCAT(CAST(tree_id AS CHAR),'_',ontology_id) cate ,ontology_name  res from ontology"+DbConfig.suffix+" " +
				"  where ontology_id is not null and  ontology_id <>'' and ontology_name is not null and ontology_name<>''   order by ontology_level asc ";
		try {
			List<Map<String,Object>> list = runner.query(sql,new MapListHandler());
	...
	public Map<String,Integer> getElementWeight()
	{
		...
		String sql ="select  CONCAT(CAST(tree_id AS CHAR),'_',element_id) cate ,element_weight  weight from element"+DbConfig.suffix+" " +
	...
	public Multimap<String, Integer> getIntentionExp(){
		...
		String sql = "SELECT  CONCAT(CAST(tree_id AS CHAR),'_',ontology_id) cate ," +
					"	intention   from ontology_resource"+DbConfig.suffix+" " +
					"  where ontology_id is not null and ontology_id<>'' and intention is not null and ontology_expression is not null and ontology_expression<>'' ";
		try {
			List<Map<String,Object>> list = runner.query(sql,new MapListHandler());
	...
	public Multimap<String, String> getAllOntoExpresssion() {
		...
		Multimap<String, String> res = ArrayListMultimap.create();
		String sql = "SELECT  CONCAT(CAST(tree_id AS CHAR),'_',ontology_id) cate ," +
					"	ontology_expression  expression from ontology_resource"+DbConfig.suffix+" " +
					"  where ontology_id is not null and ontology_id<>'' and  ontology_expression is not null and ontology_expression<>'' ";
	...
	public Multimap<String, String> geOntolongyElementMap() {
		...
		String sql = " SELECT CONCAT(CAST(ontology_tree_id  AS CHAR),'_',ontology_id) ontKey,CONCAT(CAST(element_tree_id AS CHAR),'_',element_id) elemKey FROM ontology_element"+DbConfig.suffix+" " +
				" where element_id is not null and element_id<>'' and ontology_id is not null and ontology_id <>''";
		Multimap<String,String> res = ArrayListMultimap.create();
		try {
			List<Map<String,Object>> list = runner.query(sql,new MapListHandler());			
			for(Map<String,Object> map :list)
	...
	public BidiMap getAllConceptMap() {
		BidiMap res = new DualHashBidiMap();
		String sql ="SELECT  concept_name  cate_name ,  CONCAT(CAST(tree_id AS CHAR),'_',concept_id) cate_id  from  concept"+DbConfig.suffix+" " +
				"  where concept_name is not null and concept_name<>'' and concept_id is not null and concept_id<>'' ";
		try {
			List<Map<String,Object>> list = runner.query(sql,new MapListHandler());
			...
			for(Map<String,Object> map :list)
			{
				if(!res.containsKey(map.get("concept_name")))
					res.put(map.get("concept_name").toString() , map.get("cate_id").toString());
			}
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			log.error("",e);
	...
	public BidiMap getAllElementMap() {
		BidiMap res = new DualHashBidiMap();
		String sql ="SELECT  element_name  cate_name ,  CONCAT(CAST(tree_id AS CHAR),'_',element_id) cate_id  from  element"+DbConfig.suffix+" " +
				" where element_name is not null and element_name<>'' and element_id is not null and element_id<>''";
		try {
			List<Map<String,Object>> list = runner.query(sql,new MapListHandler());
	...
	public Map<String, String> getOntoDefaultCate() {
		...
		String sql ="SELECT  CONCAT(CAST(tree_id AS CHAR),'_',ontology_id) cate ,CONCAT('1','_',default_ontology_id) ontoid from ontology"+DbConfig.suffix+"  " +
				 " where ontology_id is not null and default_ontology_id  is not null and ontology_id<>'' and default_ontology_id<>''";
	...
	public Map<String, Integer> getOntoWeight() {
		...
		Map<String, Integer> res = new HashMap<String, Integer>();
		String sql = "select  CONCAT(CAST(tree_id AS CHAR),'_',ontology_id) cate_id , weight  from ontology"+DbConfig.suffix+"  where   ontology_id is not null and weight is not null and  ontology_id<>''";
		try {
			List<Map<String, Object>> list = runner.query(sql,
					new MapListHandler());

			for (Map<String, Object> map : list) {
				res.put(map.get("cate_id").toString(), (Integer)map.get("weight"));
			}
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
	...
	public MultiBTree getOntoLevelTree()
	{
		...
		String sql ="select CONCAT(CAST(tree_id AS CHAR),'_',ontology_id) cate_id," +
				" CONCAT(CAST(tree_id AS CHAR),'_',ontology_parent_id)  pcate_id  , ontology_level  from   " +
	...
	public Multimap<String, String> getAllOntoResource() {
		...
		String sql = "select CONCAT(CAST(tree_id AS CHAR),'_',ontology_id) cate_id, ontology_expression resource " +
	...
	public Map<String, String> getInnerCodes() {
		...
		String sql=" select  CONCAT(CAST(tree_id AS CHAR),'_',ontology_id) cate , CONCAT(inner_code,'|',CAST(is_share AS CHAR)) innercode " +
		"from ontology"+DbConfig.suffix +"  where inner_code is not null and is_share is not null ";
		try {
			List<Map<String,Object>> datas = runner.query(sql, new MapListHandler());
			if(CollectionUtils.isNotEmpty(datas))
			{
				for(Map<String,Object> map :datas)
				{
					String cateid =map.get("cate").toString();
					String exp = map.get("innercode").toString();
					result.put(cateid, exp);
				}
			}
		} catch (SQLException e) {
	...
	public MultiKeyMap getAllOntoResFactors() {
		String sql ="select  CONCAT( CAST(tree_id AS CHAR),'_' ,ontology_id) cate , ontology_expression , factor   from ontology_resource"+DbConfig.suffix+"  " +
	...
	public Map<String, Integer> getAllpfilters() {
		...
		Map<String,Integer> res =  new HashMap<String, Integer>();;
		String sql ="select  CONCAT( CAST(tree_id AS CHAR),'_' ,ontology_id) cate ,pfilter   from ontology"+DbConfig.suffix+"  " +
