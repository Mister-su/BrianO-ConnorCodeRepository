...
public class MMSeg {
	...
	public Word next() throws IOException {
		...
		if (word == null) {
			...
			while (read && (data = readNext()) != -1) {
				...
				case Character.START_PUNCTUATION:// 前括号[,(,{
				case Character.END_PUNCTUATION:// 后括号],),}
				case Character.OTHER_PUNCTUATION:// 反斜杠
					/*
					 * 1. 0x3041-0x30f6 -> ぁ-ヶ //日文(平|片)假名 2. 0x3105-0x3129 ->
					 * ㄅ-ㄩ //注意符号
					 */

					if (type == Character.OTHER_PUNCTUATION && data != 47) {// 排除反斜杠意外的，比如逗号等
						read = true;
						break;
					}
					bufSentence.appendCodePoint(data);
					readChars(bufSentence, new ReadCharByType(
				...
				case Character.DECIMAL_DIGIT_NUMBER:
					...
					readChars(bufSentence, new ReadCharDigit()); // 读后面的数字,
																	// AsciiLetterOr
					...
					if (d > -1) {
						if (seg.isUnit(d)) { // 单位,如时间
							bufWord
									.add(createWord(bufSentence,
											startIdx(bufSentence) - 1,
											Word.TYPE_DIGIT)); // 先把数字添加(独立)
							...
							pushBack(d);
							if (readChars(bufSentence,
									new ReadCharByAsciiOrDigit()) > 0) { // 如果有字母或数字都会连在一起.
				...
				case Character.LETTER_NUMBER:
					...
					for (int i = 0; i < bufSentence.length(); i++) {
						bufWord.add(new Word(
								new char[] { bufSentence.charAt(i) },
								startIdx++, Word.TYPE_LETTER_NUMBER));
					}
				...
				case Character.OTHER_NUMBER:
					...
					bufWord
							.add(createWord(bufSentence, Word.TYPE_OTHER_NUMBER));
					bufSentence.setLength(0);
					break;
				default:
					// 其它认为无效字符
	 ...
	 * @return 返回读取的个数
	 * @throws IOException
	 *             {@link #readNext()} 或 {@link #pushBack()} 抛出的.
	 */
	private int readChars(StringBuilder bufSentence, ReadChar readChar)
			throws IOException {
		int num = 0;
		int data = -1;
	...
	private static class ReadCharByType extends ReadChar {
		...
		boolean isRead(int codePoint) {
			...
			if (type == Character.START_PUNCTUATION
					...
					|| (type == Character.OTHER_PUNCTUATION && codePoint == 47))//继续读新增的字符，括号+反斜杠
				type = 5;
	...
	private static boolean isRussiaLetter(int codePoint) {
		return (codePoint >= 'А' && codePoint <= 'я') || codePoint == 'Ё'
				|| codePoint == 'ё';
	}

	private static boolean isGreeceLetter(int codePoint) {
		return (codePoint >= 'Α' && codePoint <= 'Ω')
				|| (codePoint >= 'α' && codePoint <= 'ω');
	}
